...
const findNumberFromString = (mathOperationStringArray: string[]) => {
    ...
    for (let i = 0; i < mathOperationStringArray.length; i++) {
        if (mathOperationStringArray[i] === '-' && i === 0) {
            ...
            !Number.isNaN(Number(mathOperationStringArray[i])) ||
            mathOperationStringArray[i] === '.'
        ) {
            arr.push(mathOperationStringArray[i]);
        } else {
            break;
...
export const evaluateResults = (
    mathOperation: MathObject
): ReturnFromEvaluations => {
    ...
    switch (sign) {
        case MathSign.multiply: {
            ...
            historyEntry = `${firstNumber} ${
                MathSign.multiply
            } ${secondNumber} = ${result.toFixed(3)}`;
            break;
        }
